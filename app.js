...
function renderCart(){if(!cart.length){$('#cartItems').innerHTML='<p class="checkout-note">Sua sacola está vazia.</p>';$('#cartTotal').textContent='$0.00';return}$('#cartItems').innerHTML=cart.map((x,i)=>{const p=products.find(z=>z.slug===x.slug);if(!p)return'';return `<div class="cart-row"><img src="${imagesFor(p,x.lang||'pt')[0]||''}" alt=""><div><strong>${p.title}</strong><p>Tamanho ${x.size} · ${labelForLang(x.lang||'pt')} · Qtd. ${x.qty}</p><p>${money(p.price)}</p></div><button class="remove" data-remove="${i}">Remover</button></div>`}).join('');$('#cartTotal').textContent=money(cartTotal());$$('[data-remove]').forEach(b=>b.onclick=()=>{cart.splice(Number(b.dataset.remove),1);persist();renderCart()})}
...
function orderText(method){const lines=cart.map(x=>{const p=products.find(z=>z.slug===x.slug);return p?`• ${p.title} | Tam. ${x.size} | Idioma: ${labelForLang(x.lang||'pt')} | Qtd. ${x.qty} | ${money((p.price||0)*x.qty)}`:''}).filter(Boolean);return ['Olá! Quero finalizar meu pedido na JESUSFIRST.','',...lines,'',`Total: ${money(cartTotal())}`,`Forma de pagamento: ${method}`,'Entrega: Estados Unidos'].join('\n')}
